...
export async function getDashboard(currentSlug='lap18') {
  ...
  const [currentStats, previousEvents, registrations] = await Promise.all([
    ...
    sql`SELECT r.id,r.status,r.created_at,c.first_name,c.last_name,c.phone,c.company,e.name event_name,e.slug,
      ...
      pg.payment_method,pg.code payment_code,pg.expected_participants,r.answers->>'photoUrl' photo_url,
...
function normalizeCRMFilters(filters={}) {
  ...
  return {eventSlug,q,company,registrationStatus,paymentStatus,paymentMethod};
}

export async function getCRM(filters={}) {
  const sql=db();
  const normalized=normalizeCRMFilters(filters);
  const {eventSlug,q,company,registrationStatus,paymentStatus,paymentMethod}=normalized;
  ...
  return {events,rows,total,page,limit,pages:Math.max(1,Math.ceil(total/limit)),filters:normalized};
}

export async function getCRMExport(filters={}) {
  const sql=db();
  const {eventSlug,q,company,registrationStatus,paymentStatus,paymentMethod}=normalizeCRMFilters(filters);
  const likeQ=`%${q}%`;
  const likeCompany=`%${company}%`;
