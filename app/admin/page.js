...
export default async function Admin(){
  ...
  return <div className="admin-shell">
    <header className="admin-top"><div className="admin-wrap admin-top-inner"><div><span className="admin-kicker">GESTÃO DE ALTA PERFORMANCE</span><div className="admin-brand">AXIS · CRM</div></div><nav className="adminnav"><a href="/admin">Dashboard</a><a href="/lap18" target="_blank">Ver LAP 18</a><form method="post" action="/api/admin/logout"><button className="admin-logout">Sair</button></form></nav></div></header>
    <main className="admin-wrap admin-main">
      <section className="admin-hero"><div><span className="admin-kicker">PAINEL ADMINISTRATIVO</span><h1>{d.currentEvent?.name||'LAP 18'}</h1><p>Inscrições, pagamentos e operação do evento atual em uma única visão.</p></div><div className="admin-event-status"><span>EVENTO ATUAL</span><strong>{d.currentEvent?.status||'—'}</strong></div></section>
      ...
      <section className="admin-stats">
        ...
        <article className="admin-stat-card"><span>Pagamentos pendentes</span><strong>{d.stats.pending_payments}</strong><small>aguardando validação</small></article>
      </section>

      <section className="admin-panel"><div className="admin-panel-head"><div><span className="admin-kicker">LAP 18</span><h2>Inscrições do evento atual</h2></div><span className="admin-count">{d.registrations.length} registro{d.registrations.length===1?'':'s'}</span></div>
        {d.registrations.length?<div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Participante</th><th>Empresa</th><th>Status</th><th>Pagamento</th></tr></thead><tbody>{d.registrations.map(r=><tr key={r.id}><td><strong>{r.first_name} {r.last_name}</strong><small>{r.phone}</small></td><td>{r.company||'—'}</td><td><span className={`admin-pill admin-pill-${r.status||'new'}`}>{r.status||'new'}</span></td><td><PaymentCell r={r}/></td></tr>)}</tbody></table></div>:<div className="admin-empty"><strong>Nenhuma inscrição real ainda.</strong><span>Os contadores do LAP 18 estão prontos para começar do zero.</span></div>}
      </section>

      <details className="admin-archive"><summary><span><strong>Eventos anteriores</strong><small>Ocultos para manter o foco no evento atual</small></span><span>{d.previousEvents.length}</span></summary><div className="admin-archive-body">{d.previousEvents.length?<table className="admin-table"><thead><tr><th>Evento</th><th>Status</th><th>Inscrições</th><th>Link</th></tr></thead><tbody>{d.previousEvents.map(e=><tr key={e.id}><td><strong>{e.name}</strong></td><td><span className="admin-pill">{e.status}</span></td><td>{e.registration_count}</td><td><a href={`/${e.slug}`} target="_blank">/{e.slug}</a></td></tr>)}</tbody></table>:<div className="admin-empty"><span>Nenhum evento anterior cadastrado.</span></div>}</div></details>
    </main>
  </div>
}
