...
function PhotoCell({r}){return r.photo_url?<a className="proof-link" href={r.photo_url} target="_blank" rel="noopener noreferrer" download>Baixar foto ↗</a>:<small>Sem foto</small>}
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-wrap admin-main">
      ...
      <section className="admin-panel"><div className="admin-panel-head"><div><span className="admin-kicker">LAP 18</span><h2>Inscrições <em>do evento atual</em></h2></div><div className="admin-panel-actions"><a className="admin-text-link" href="/admin/crm?event=lap18">Abrir no CRM</a><span className="admin-count">{d.registrations.length} registro{d.registrations.length===1?'':'s'}</span></div></div>
        {d.registrations.length?<div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Participante</th><th>Empresa</th><th>Status</th><th>Pagamento</th><th>Foto #euvou</th></tr></thead><tbody>{d.registrations.map(r=><tr key={r.id}><td><strong>{r.first_name} {r.last_name}</strong><small>{r.phone}</small></td><td>{r.company||'—'}</td><td><span className={`admin-pill admin-pill-${r.status||'new'}`}>{r.status||'new'}</span></td><td><PaymentCell r={r}/></td><td><PhotoCell r={r}/></td></tr>)}</tbody></table></div>:<div className="admin-empty"><strong>Nenhuma inscrição registrada até o momento.</strong><span>Novos participantes aparecerão aqui automaticamente.</span></div>}
      </section>

      <details className="admin-archive"><summary><span><strong>Eventos <em>anteriores</em></strong><small>Histórico preservado e recolhido para manter o foco no LAP 18.</small></span><span className="admin-archive-count">{d.previousEvents.length}</span></summary><div className="admin-archive-body">{d.previousEvents.length?<table className="admin-table"><thead><tr><th>Evento</th><th>Status</th><th>Inscrições</th><th>Acesso</th></tr></thead><tbody>{d.previousEvents.map(e=><tr key={e.id}><td><strong>{e.name}</strong><small>/{e.slug}</small></td><td><span className="admin-pill">{e.status}</span></td><td>{e.registration_count}</td><td><div className="admin-row-actions"><a className="admin-text-link" href={`/admin/crm?event=${encodeURIComponent(e.slug)}`}>Ver inscritos</a><a href={`/${e.slug}`} target="_blank">Ver página ↗</a></div></td></tr>)}</tbody></table>:<div className="admin-empty"><span>Nenhum evento anterior cadastrado.</span></div>}</div></details>
    </main>
  </div>
}
