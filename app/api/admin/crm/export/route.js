...
export async function GET(request){
 ...
 const url=new URL(request.url);const filters={eventSlug:url.searchParams.get('event')||'',q:url.searchParams.get('q')||'',company:url.searchParams.get('company')||'',registrationStatus:url.searchParams.get('registrationStatus')||'',paymentStatus:url.searchParams.get('paymentStatus')||'',paymentMethod:url.searchParams.get('paymentMethod')||''};
 ...
 const buffer=await workbook.xlsx.writeBuffer();const today=new Date().toISOString().slice(0,10);const scope=filters.eventSlug?safeFilenamePart(filters.eventSlug):'todos-eventos';const filename=`axis-crm-${scope}-${today}.xlsx`;return new Response(buffer,{status:200,headers:{'Content-Type':'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','Content-Disposition':`attachment; filename="${filename}"`,'Cache-Control':'no-store'}});
}
