...
import {getCRMExportByEvents} from '@/lib/crm-export';
...
export async function GET(request){
 ...
 const rows=await getCRMExportByEvents(filters);const workbook=new ExcelJS.Workbook();workbook.creator='AXIS CRM';workbook.created=new Date();const sheet=workbook.addWorksheet('CRM',{views:[{state:'frozen',ySplit:1}]});sheet.columns=selectedColumns.map(({header,key,width})=>({header,key,width}));
